class Solution(object):
    def numIslands2_1(self, m, n, positions):
        """
        :type m: int
        :type n: int
        ...
        for v in positions:
            ...
            if res == []:
                ...
                islands.add((v[0], v[1]))
                res.append(curNo)
                print('1: v: {} res: {} islands: {}'.format(v, res, islands))
                continue
            # old position
            if (v[0], v[1]) in islands:
                res.append(curNo)
                print('2: v: {} res: {} islands: {}'.format(v, res, islands))
                continue
            # a new position
            count = 0
            for d in dirs:
                ...
                if 0 <= x <= m-1 and 0 <= y <= n-1:
                    # union two islands
                    if (x, y) in islands:
                        # the connected islands may be one islands
                        # so here is an error. have to tell if there are the same islands
                        count += 1
            print('count: {}'.format(count))
            curNo = max(curNo - count + 1, 1)
            res.append(curNo)
            islands.add((v[0], v[1]))
            print('3: v: {} res: {} islands: {}'.format(v, res, islands))
        return res
    ...
    def numIslands2(self, m, n, positions):
        ...
        """
        # python 2 timeout:
        # python 3 : Runtime: 3210 ms, faster than 5.03% of Python3 online submissions for Number of Islands II.
        # Memory Usage: 19.5 MB, less than 79.29% of Python3 online submissions for Number of Islands II.
        # time:o(len(positions)*m*n)  size of islands = m*n space:o(len(positions))
        # improvement of numIslands2_1: have to save the island id
        res = []
        curNo = 0
        islands = {}
        isLandId = 0
        ...
        for v in positions:
            # Old position
            # list is unhashable; can't be element of set
            if (v[0], v[1]) in islands:
                res.append(curNo)
                #print('2: v: {} res: {} islands: {}'.format(v, res, islands))
                continue
            # a new position
            # save the key of islands
            count = set()
            for d in dirs:
                x, y = v[0] + d[0], v[1] + d[1]
                if 0 <= x <= m-1 and 0 <= y <= n-1 and (x, y) in islands:
                    # union two islands: save the island id
                    count.add(islands[(x, y)])
            # len(count): the different islands connected
            l = len(count)
            # new island
            if l == 0:
                newId = isLandId
                ...
                for id in count:
                    newId = id
                    break
                #print('newId: {} islands: {} v: {}'.format(newId, islands, v))
                for k, val in islands.items():
                    if val in count:
                        islands[k] = newId
            islands[(v[0], v[1])] = newId
            curNo = curNo - l + 1
            res.append(curNo)
        return res
